...
//            Log::Write(Log::Level::Info, Fmt("Passthrough-Create Geometry Instance:%d", m_geometryInstance));
    ...
    void InitializeApplication() override {
        ...
        m_application->setHapticCallback((void*)this, [](void *arg, int controllerIndex, float amplitude, float seconds, float frequency) {
            ...
            vibration.duration = static_cast<XrDuration>(seconds * 1000000000.0f);  // nanoseconds
    ...
    void PollActions() override {
        ...
        // the OpenXR thread, where the session and action handles are valid.
        double haptic[4] = {};
        while (eva_bridge_poll_haptic(haptic)) {
            const int hand = static_cast<int>(haptic[0]);
            if (hand < Side::LEFT || hand > Side::RIGHT) {
                continue;
            }
            XrHapticVibration vibration{XR_TYPE_HAPTIC_VIBRATION};
            vibration.amplitude = static_cast<float>(std::max(0.0, std::min(1.0, haptic[1])));
            vibration.duration = static_cast<XrDuration>(std::max(1.0, std::min(1000.0, haptic[2])) * 1000000.0);
            vibration.frequency = XR_FREQUENCY_UNSPECIFIED;
            XrHapticActionInfo hapticInfo{XR_TYPE_HAPTIC_ACTION_INFO};
            hapticInfo.action = m_input.hapticAction;
